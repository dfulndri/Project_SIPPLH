...
@push('styles')
<style>
.tf-step { display:inline-flex; align-items:center; gap:4px; padding:4px 10px; font-size:.68rem; font-weight:500; background:#f3f4f6; color:var(--muted); }
...
<div class="page-hd d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
  ...
  <div class="d-flex gap-2">
    ...
    <a href="{{ route('pengawas.verifikasi.create', ['pengaduan_id' => $pengaduan->id]) }}" class="btn btn-sm btn-maroon"><i class="bi bi-clipboard-check me-1"></i>Mulai Verifikasi</a>
    @endif
    <a href="{{ route('pengawas.tugas.index') }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Kembali</a>
  </div>
</div>
...
<div class="row g-3">
  <div class="col-md-8">
    ...
    <div class="card-panel mb-3">
      ...
      <div class="cp-body">
        <div class="row">
          <div class="col-md-4"><div class="info-label">Tanggal</div><div class="info-value">{{ $pengaduan->tanggal_pengaduan->format('d M Y') }}</div></div>
          <div class="col-md-4"><div class="info-label">Status</div><div class="info-value"><span class="badge bg-{{ $pengaduan->status_badge }}">{{ $pengaduan->status_label }}</span></div></div>
          <div class="col-md-4"><div class="info-label">Kecamatan</div><div class="info-value">{{ $pengaduan->kecamatan?->nama_kecamatan ?? '—' }}</div></div>
          <div class="col-12">
            <div class="info-label">Jenis Aduan</div>
            <div class="info-value">@foreach($pengaduan->jenis_aduan_labels as $ja)<span class="badge-kat me-1">{{ $ja }}</span>@endforeach</div>
          </div>
          <div class="col-12">
    ...
    <div class="card-panel mb-3">
      ...
      <div class="cp-body">
        <div class="row">
          ...
          <div class="col-md-4"><div class="info-label">Catatan</div><div class="info-value">{{ $pengaduan->disposisi->catatan }}</div></div>
          @endif
        </div>
      </div>
    </div>
    ...
    <div class="card-panel mb-3">
      <div class="cp-head">
        ...
        <a href="{{ route('pengawas.verifikasi.edit', $pengaduan->verifikasi) }}" class="btn btn-xs btn-outline-maroon"><i class="bi bi-pencil"></i> Edit</a>
        @endif
      </div>
      <div class="cp-body">
        <div class="row">
          ...
          <div class="col-12"><div class="info-label">Fakta Temuan</div><div class="info-value" style="white-space:pre-line">{{ $pengaduan->verifikasi->fakta_temuan }}</div></div>
          @endif
          @if($pengaduan->verifikasi->saran_tindak_lanjut)
          <div class="col-12"><div class="info-label">Saran Tindak Lanjut</div><div class="info-value" style="white-space:pre-line">{{ $pengaduan->verifikasi->saran_tindak_lanjut }}</div></div>
          @endif
        </div>
        @if($pengaduan->verifikasi->dokumentasiFoto?->isNotEmpty())
        <div class="row g-2 mt-2">
          @foreach($pengaduan->verifikasi->dokumentasiFoto as $foto)
          <div class="col-md-3"><img src="{{ asset('storage/' . $foto->path_file) }}" class="img-fluid rounded" style="max-height:120px;width:100%;object-fit:cover"></div>
          @endforeach
        </div>
        @endif
      </div>
  ...
  <div class="col-md-4">
    <div class="card-panel mb-3">
      ...
      <div class="cp-body">
        <div class="info-label">Nama</div><div class="info-value">{{ $pengaduan->pelapor?->nama_display }}</div>
        <div class="info-label">Jenis</div><div class="info-value"><span class="badge bg-info text-dark">{{ $pengaduan->pelapor?->jenis_label }}</span></div>
        @if($pengaduan->pelapor?->no_telp)<div class="info-label">Telp</div><div class="info-value">{{ $pengaduan->pelapor->no_telp }}</div>@endif
      </div>
    </div>
    <div class="card-panel mb-3">
      <div class="cp-head"><div class="cp-title"><i class="bi bi-exclamation-triangle me-1"></i>Terlapor</div></div>
      <div class="cp-body">
        <div class="info-label">Nama</div><div class="info-value">{{ $pengaduan->terlapor?->nama ?? '—' }}</div>
        <div class="info-label">Jenis</div><div class="info-value"><span class="badge bg-warning text-dark">{{ $pengaduan->terlapor?->jenis_label }}</span></div>
