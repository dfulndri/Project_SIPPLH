...
<div class="card-panel">
  <div class="table-responsive">
    <table class="table sipplh-table">
      <thead><tr><th>No. Pengaduan</th><th>Terlapor</th><th>Tanggal Verifikasi</th><th>Status</th><th>Aksi</th></tr></thead>
      <tbody>
        ...
        <tr>
          ...
          <td>
            @if($v->status === 'draft')
            <a href="{{ route('pengawas.verifikasi.edit', $v) }}" class="btn btn-xs btn-outline-secondary"><i class="bi bi-pencil"></i></a>
            @endif
            <a href="{{ route('pengawas.tugas.show', $v->pengaduan_id) }}" class="btn btn-xs btn-outline-secondary"><i class="bi bi-eye"></i></a>
          </td>
        </tr>
        @empty
        <tr><td colspan="5"><div class="empty-state"><i class="bi bi-clipboard-check"></i>Belum ada verifikasi</div></td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
