...
<div class="page-hd d-flex align-items-start justify-content-between flex-wrap gap-3">
  <div>
    ...
    <p class="page-stl">Diterbitkan {{ $ba->tanggal_terbit->format('d F Y') }} &middot; oleh {{ $ba->pembuat?->name }}</p>
  </div>
...
<div class="card-panel mb-3" style="border:2px solid var(--maroon)">
  ...
  <div class="cp-body">
    <h2 style="text-align:center;font-size:1rem;font-weight:700;text-transform:uppercase;margin:1rem 0;letter-spacing:.05em">
    ...
    {{-- SECTION A --}}
    <h6 style="font-weight:700;color:var(--maroon);border-bottom:1px solid var(--border);padding-bottom:4px;margin-bottom:10px">
    ...
    <div class="row g-2 mb-4" style="font-size:.82rem">
      @foreach([
        ['Nama Penanggung Jawab',  $pj->nama_pj],
        ['Jabatan',                 $pj->jabatan_pj ?: '—'],
        ['Nama Perusahaan',        $pj->nama_perusahaan],
        ['Bidang Usaha / Kegiatan',$pj->bidang_usaha ?: '—'],
        ['Kode KBLI',              $pj->kbli ?: '—'],
        ['NIB',                    $pj->nib ?: '—'],
        ['No. Telepon',            $pj->no_telp ?: '—'],
        ['Email',                  $pj->email ?: '—'],
      ] as [$lbl,$val])
      <div class="col-6 col-md-3">
        <div style="font-size:.68rem;color:var(--muted);text-transform:uppercase;letter-spacing:.04em">{{ $lbl }}</div>
        <div style="margin-top:2px;border-bottom:1px solid var(--border);padding-bottom:3px">{{ $val }}</div>
      </div>
      @endforeach
      @if($pj->alamat_perusahaan)
      <div class="col-12">
        <div style="font-size:.68rem;color:var(--muted);text-transform:uppercase;letter-spacing:.04em">Alamat Perusahaan</div>
        <div style="margin-top:2px;border-bottom:1px solid var(--border);padding-bottom:3px">{{ $pj->alamat_perusahaan }}</div>
      </div>
      @endif
    </div>
    @else
    ...
    <div class="mb-4 p-3 rounded" style="background:#fafafa;border:1px solid var(--border);font-size:.875rem;line-height:1.8;white-space:pre-line;min-height:80px">
    ...
    <div class="p-3 rounded mb-4" style="background:rgba(106,0,0,.04);border:1px solid rgba(106,0,0,.15);font-size:.82rem;line-height:1.7">
      <i class="bi bi-info-circle-fill me-1" style="color:var(--maroon)"></i>
      Berdasarkan hasil verifikasi lapangan di atas, penanggung jawab usaha/kegiatan <strong>wajib menindaklanjuti
      rekomendasi tersebut dalam jangka waktu 14 (empat belas) hari kalender</strong>, paling lambat tanggal
      ...
      <div class="col-12 col-md-5">
        <div style="text-align:center">
          ...
          <div style="margin-top:50px;border-top:1px solid #333;padding-top:4px;font-size:.78rem">
            ...
            @if($t->nip)<div style="color:var(--muted)">NIP: {{ $t->nip }}</div>@endif
          </div>
          @empty
      ...
      <div class="col-12 col-md-5">
        <div style="text-align:center">
          ...
          <div style="margin-top:50px;border-top:1px solid #333;padding-top:4px;font-size:.78rem">
            ...
            @if($pj?->jabatan_pj)<div style="color:var(--muted)">{{ $pj->jabatan_pj }}</div>@endif
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="mt-4 pt-3 border-top text-center" style="font-size:.72rem;color:var(--muted)">
      ...
      Verifikasi di: <a href="{{ route('ba.verify', $ba->qr_code_token) }}" target="_blank">{{ url('/verify/' . $ba->qr_code_token) }}</a>
    </div>
  </div>
</div>
...
<div class="card-panel">
  <div class="cp-head"><div class="cp-title"><i class="bi bi-images me-1 text-muted"></i> Dokumentasi Foto</div></div>
  <div class="cp-body">
    <div class="row g-2">
      ...
      <div class="col-6 col-md-3 col-lg-2">
        <div style="border-radius:6px;overflow:hidden;border:1px solid var(--border)">
          ...
          @if($foto->keterangan)
          <div style="padding:3px 6px;font-size:.65rem;color:var(--muted)">{{ $foto->keterangan }}</div>
          @endif
        </div>
      </div>
      @endforeach
