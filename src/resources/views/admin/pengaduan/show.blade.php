    ...
    <div class="page-hd d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
        ...
        <div class="d-flex gap-2">
            @if (in_array($pengaduan->status, ['pengaduan_baru', 'menunggu_disposisi']))
                <a href="{{ route('admin.pengaduan.edit', $pengaduan) }}" class="btn btn-sm btn-outline-secondary"><i
                        class="bi bi-pencil me-1"></i>Edit</a>
                <a href="{{ route('admin.disposisi.create', ['pengaduan_id' => $pengaduan->id]) }}"
                    class="btn btn-sm btn-maroon"><i class="bi bi-send me-1"></i>Disposisikan</a>
            @endif
            <a href="{{ route('admin.pengaduan.index') }}" class="btn btn-sm btn-outline-secondary"><i
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="info-value"><span
                                    class="badge bg-{{ $pengaduan->sumber_laporan == 'span_lapor' ? 'info' : 'secondary' }}">{{ $pengaduan->sumber_laporan == 'span_lapor' ? 'SPAN LAPOR' : 'Manual' }}</span>
                            </div>
                        </div>
                        <div class="col-md-4">
                        ...
                        @if ($pengaduan->koordinat_lat)
                            <div class="col-md-4">
                                ...
                                <div class="info-value">{{ $pengaduan->koordinat_lat }}, {{ $pengaduan->koordinat_lng }}
                                </div>
                            </div>
                        @endif
                        @if ($pengaduan->dokumen_pendukung)
                            <div class="col-12">
                                ...
                                <div class="info-value"><a href="{{ asset('storage/' . $pengaduan->dokumen_pendukung) }}"
                                        target="_blank" class="btn btn-xs btn-outline-maroon"><i
                                            class="bi bi-download me-1"></i>Unduh Dokumen</a></div>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            @if ($pengaduan->disposisi)
                <div class="card-panel mb-3">
                    ...
                    <div class="cp-body">
                        <div class="row">
                            <div class="col-md-4">
                                <div class="info-label">Pengawas</div>
                                <div class="info-value"><strong>{{ $pengaduan->disposisi->pengawas?->name }}</strong></div>
                            </div>
                            <div class="col-md-4">
            ...
            @if ($pengaduan->verifikasi)
                <div class="card-panel mb-3">
                    <div class="cp-head">
                        ...
                        <a href="{{ route('admin.verifikasi.show', $pengaduan->verifikasi) }}"
                            class="btn btn-xs btn-outline-maroon">Detail <i class="bi bi-arrow-right"></i></a>
                    </div>
                    <div class="cp-body">
                        <div class="row">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="info-value">{{ $pengaduan->verifikasi->pembuat?->name }}</div>
                            </div>
                            @if ($pengaduan->verifikasi->beritaAcara)
                                <div class="col-md-3">
                                    <div class="info-label">Berita Acara</div>
                                    <div class="info-value"><a
                                            href="{{ route('admin.berita-acara.show', $pengaduan->verifikasi->beritaAcara) }}"
                                            class="btn btn-xs btn-outline-maroon">{{ $pengaduan->verifikasi->beritaAcara->nomor_ba }}</a>
                                    </div>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="cp-body">
                        @foreach ($pengaduan->tindakLanjut as $tl)
                            <div class="border rounded p-3 mb-2" style="font-size:.84rem">
                                ...
                                @if ($tl->hasil)
                                    <p class="text-muted mb-0"><strong>Hasil:</strong> {{ $tl->hasil }}</p>
                                @endif
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="col-md-4">
            <div class="card-panel mb-3">
                ...
                <div class="cp-body">
                    ...
                    @if ($pengaduan->pelapor?->nik)
                        <div class="info-label">NIK</div>
                        <div class="info-value">{{ $pengaduan->pelapor->nik }}</div>
                    @endif
                    @if ($pengaduan->pelapor?->no_telp)
                        <div class="info-label">No. Telp</div>
                        <div class="info-value">{{ $pengaduan->pelapor->no_telp }}</div>
                    @endif
                    @if ($pengaduan->pelapor?->email)
                        <div class="info-label">Email</div>
                        <div class="info-value">{{ $pengaduan->pelapor->email }}</div>
                    @endif
                    @if ($pengaduan->pelapor?->alamat)
                        <div class="info-label">Alamat</div>
                        <div class="info-value">{{ $pengaduan->pelapor->alamat }}</div>
                    @endif
                </div>
            </div>
            ...
            <div class="card-panel mb-3">
                ...
                <div class="cp-body">
                    ...
                    @if ($pengaduan->terlapor?->jenis_usaha)
                        <div class="info-label">Jenis Usaha</div>
                        <div class="info-value">{{ $pengaduan->terlapor->jenis_usaha }}</div>
                    @endif
                    @if ($pengaduan->terlapor?->alamat)
                        <div class="info-label">Alamat</div>
                        <div class="info-value">{{ $pengaduan->terlapor->alamat }}</div>
                    @endif
                    @if ($pengaduan->terlapor?->no_telp)
                        <div class="info-label">No. Telp</div>
                        <div class="info-value">{{ $pengaduan->terlapor->no_telp }}</div>
                    @endif
                </div>
            </div>
            ...
            <div class="card-panel">
                <div class="cp-head">
                    <div class="cp-title"><i class="bi bi-lightning me-1"></i>Aksi Cepat</div>
                </div>
                <div class="cp-body">
                    <form action="{{ route('admin.pengaduan.status', $pengaduan) }}" method="POST">
                        ...
                        <div class="mb-2">
                            ...
                            <select name="status" class="form-select form-select-sm">
                                @foreach (\App\Models\Pengaduan::$statusList as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ $pengaduan->status == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-2">
                        ...
                        <button type="submit" class="btn btn-sm btn-maroon w-100"><i
                    ...
                    @if (!$pengaduan->disposisi && in_array($pengaduan->status, ['pengaduan_baru', 'menunggu_disposisi']))
                        <a href="{{ route('admin.disposisi.create', ['pengaduan_id' => $pengaduan->id]) }}"
                            class="btn btn-sm btn-outline-maroon w-100 mb-2"><i
                                class="bi bi-send me-1"></i>Disposisikan</a>
                    @endif
                    @if (!$pengaduan->verifikasi && in_array($pengaduan->status, ['didisposisikan']))
                        <a href="{{ route('admin.verifikasi.create', ['pengaduan_id' => $pengaduan->id]) }}"
                            class="btn btn-sm btn-outline-maroon w-100 mb-2"><i
                                class="bi bi-clipboard-check me-1"></i>Buat Verifikasi</a>
                    @endif
                    @if (in_array($pengaduan->status, ['verifikasi_selesai', 'tindak_lanjut']))
                        <a href="{{ route('admin.tindak-lanjut.create', ['pengaduan_id' => $pengaduan->id]) }}"
                            class="btn btn-sm btn-outline-maroon w-100 mb-2"><i class="bi bi-arrow-repeat me-1"></i>Tindak
                            Lanjut</a>
                    @endif

                    <form action="{{ route('admin.pengaduan.destroy', $pengaduan) }}" method="POST"
                        onsubmit="return confirm('Hapus pengaduan ini secara permanen?')">
                        @csrf @method('DELETE')
